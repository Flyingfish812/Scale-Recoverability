...
# s01_fix_figure3 引擎与 fig03 同目录 (_canonical)
...
def main() -> None:
    t_start = time.time()
    print("=" * 72)
    print("  Figure 3 (Equal-GER) — 优先读 statistics 产物画图")
    print("=" * 72)

    # 数据/图分离: 数据由 compute_equal_ger_42000 重算 (跳过机制);
    # 本脚本只读 statistics/fig03_data.json 画图。--recompute 强制重算。
    STATS = ROOT / "artifacts" / "derived" / "main" / "statistics"
    fig3_json = STATS / "fig03_data.json"
    if fig3_json.exists() and "--recompute" not in sys.argv:
        data = json.loads(fig3_json.read_text(encoding="utf-8"))
        generate_figure3(data["all_pairs"], data["representative_pair"],
                         data["paired_statistics"], data["cluster_bootstrap_ci"],
        ...
        print(f"  [fig03] 读 {fig3_json.relative_to(ROOT)} → 画图完成")
        return 0

    print("\n  未找到 statistics/fig03_data.json 或 --recompute, 执行完整重算...")
    print("=" * 72)
    ...
    result = {
        ...
        "supersedes": "artifacts/derived/main/statistics/fig03_data.json (含废弃 AdamW-Ridge 配对)",
    }
    ...
    s33_report = {
        ...
        "supersedes": "artifacts/derived/main/statistics/s33_strict_equal_ger.json (含废弃 AdamW-Ridge 配对)",
        "evaluation": (
